...
s32 ashtech::measure(void)
{
    ...
    uint64_t time_started = core::get_timestamp();
...
s32 ashtech::handle_message(s32 len)
{
	...
	if ((memcmp(_rx_buffer + 3, "ZDA,", 3) == 0) && (uiCalcComma == 6)) {
		...
		*/
		...
		s32 day = 0, month = 0, year = 0, local_time_off_hour __attribute__((unused)) = 0, local_time_off_min __attribute__((unused)) = 0;
		...
		*/
		...
		s32 num_of_sv __attribute__((unused)) = 0, fix_quality = 0;
